...
def fetch_group_emails(
    ...
    if not use_mock and not gmail_token and not gmail_secret:
        ...
        if not os.path.exists(token_path) and not os.path.exists(secrets_path):
            logger.warning(
                f"No Gmail API credentials found. Looking for token.json or secrets.json in .secrets directory"
            )
    ...
    try:
        ...
        if not creds or not hasattr(creds, "authorize"):
            ...
            logger.warning(
                "Ensure GMAIL_TOKEN environment variable is set or token.json file exists"
            )
            mock_email = {
                "from_email": "sender@example.com",
                "to_email": email_address,
        ...
        for message in messages:
            try:
                ...
                if not should_process:
                    if is_from_user:
                        logger.debug(
                            f"Skipping message {message['id']}: sent by the user"
                        )
                    elif not is_latest_in_thread:
                        logger.debug(
                            f"Skipping message {message['id']}: not the latest in thread"
                        )
                ...
                if should_process:
                    ...
                    logger.info(
                        f"Processing message {message['id']} from thread {thread_id}"
                    )
...
def send_email(
    ...
    try:
        ...
        creds = get_credentials(
            gmail_token=os.getenv("GMAIL_TOKEN"), gmail_secret=os.getenv("GMAIL_SECRET")
        )
        service = build("gmail", "v1", credentials=creds)
        ...
        try:
            ...
            logger.warning(
                f"Could not retrieve original message with ID {email_id}. Error: {str(e)}"
            )
        ...
        for date_str in dates:
            ...
            for event in events:
                ...
                if "T" in start:  # dateTime format
                    ...
                    result += (
                        f"  - {start_display} - {end_display}: {event['summary']}\n"
                    )
            ...
            if "all-day" in [slot[0] for slot in busy_slots]:
                ...
                work_start = datetime(
                    year=int(year), month=int(month), day=int(day), hour=9, minute=0
                )
                work_end = datetime(
                    year=int(year), month=int(month), day=int(day), hour=17, minute=0
                )
...
class ScheduleMeetingInput(BaseModel):
    ...
    timezone: str = Field(
        default="America/Los_Angeles", description="Timezone for the meeting"
    )
...
def schedule_meeting_tool(
    ...
    try:
        success = send_calendar_invite(
            attendees, title, start_time, end_time, organizer_email, timezone
        )
