...
async def fetch_and_process_emails(args):
    ...
    try:
        ...
        if args.minutes_since > 0:
            ...
            after = int(
                (datetime.now() - timedelta(minutes=args.minutes_since)).timestamp()
            )
            query += f" after:{after}"
        ...
        for i, message_info in enumerate(messages):
            ...
            message = (
                service.users()
                .messages()
                .get(userId="me", id=message_info["id"])
                .execute()
            )
            ...
            thread_id, run = await ingest_email_to_langgraph(
                email_data, args.graph_name, url=args.url
            )
...
def parse_args():
    ...
    parser.add_argument(
        "--email", type=str, required=True, help="Email address to fetch messages for"
    )
    parser.add_argument(
        "--minutes-since",
        type=int,
    ...
    parser.add_argument(
        "--early", action="store_true", help="Early stop after processing one email"
    )
    parser.add_argument(
        "--include-read",
        action="store_true",
    ...
    parser.add_argument(
        "--skip-filters", action="store_true", help="Skip filtering of emails"
    )
