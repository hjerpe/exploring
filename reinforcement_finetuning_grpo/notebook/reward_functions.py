...
def uses_previous_feedback(prompt: str, completion: str, example: dict) -> int:
    ...
    try:
        ...
        for idx, letter in enumerate(guess):
            ...
            if (
                letter in correct_letter_to_position
                and idx in correct_letter_to_position[letter]
            ):
                reward += 0.2
            # Reward if letter known to be in word is used in a new position
            elif (
                letter in valid_letter_to_position
                and idx not in valid_letter_to_position[letter]
            ):
                reward += 0.1
            # Penalize reuse of known-in-word letter in same position (not exploring)
            elif (
                letter in valid_letter_to_position
                and idx in valid_letter_to_position[letter]
            ):
                reward -= 0.2
...
def guess_value(prompt: str, completion: str, example: dict) -> int:
    ...
    def compute_normalized_information_gain(all_candidate_words, past_guesses, guess):
        ...
        for word in candidates:
            ...
            feedback_pattern = "".join(
                "1" if "✓" in fb else ("0" if "-" in fb else "x") for fb in feedback
            )
        ...
        normalized_expected_gain = (
            expected_gain / current_entropy if current_entropy > 0 else 0
        )
        normalized_max_gain = (
            max_info_gain / current_entropy if current_entropy > 0 else 0
        )
    ...
    try:
        ...
        normalized_expected_gain, _ = compute_normalized_information_gain(
            word_list["Word"].values, past_guess_history, guess
        )
