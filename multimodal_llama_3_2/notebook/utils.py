...
def llama32(messages, model_size=11, max_retries=5):
    ...
    while retries < max_retries:
        ...
        if res.status_code == 200:
            ...
            raise Exception(f"Failed with status code: {res.status_code}, {res.text}")
...
def llama31(prompt_or_messages, model_size=8, temperature=0, raw=False, debug=False):
    ...
    if isinstance(prompt_or_messages, str):
        ...
        payload = {"model": model, "temperature": temperature, "prompt": prompt}
    else:
